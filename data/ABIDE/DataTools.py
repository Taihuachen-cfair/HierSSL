...
class DataTools:
    def __init__(self, root_folder, txt_path):
        self.root_folder = root_folder
        self.data_folder = os.path.join(root_folder, 'ABIDE_pcp/cpac/filt_noglobal')
        self.phenotype_path = os.path.join(root_folder, 'ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv')
        self.txt_path = txt_path
        self.subject_IDs = self._get_ids().tolist()

    def fetch_filenames(self, subject_IDs, file_type):
        """
        ...
        for i in range(len(subject_IDs)):
            ...
            # os.chdir(os.path.join(self.data_folder, subject_IDs[i]))
            try:
    ...
    def get_timeseries(self, subject_list, atlas_name):
        """
            subject_list : list of short subject IDs in string format
    ...
    def subject_connectivity(self, timeseries, subject, atlas_name, kind, save=True, save_path=None):
        """
    ...
    def _get_ids(self, num_subjects=None):
        """
        return:
            subject_IDs    : list of all subject IDs
        """

        subject_IDs = np.genfromtxt(self.txt_path, dtype=str)
    ...
    def get_subject_score(self, subject_list, score):
        ...
        with open(self.phenotype_path) as csv_file:
    ...
    def site_percentage(self, train_ind, perc, subject_list):
        ...
        sites = self.get_subject_score(train_list, score='SITE_ID')
    ...
    def create_affinity_graph_from_scores(self, scores, subject_list):
        """
        ...
        for l in scores:
            label_dict = self.get_subject_score(subject_list, l)
