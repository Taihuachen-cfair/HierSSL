...
class OutputLayer(nn.Module):
    def __init__(self, d_in, d_hidden, hyperpm):
        super(OutputLayer, self).__init__()
        self.hyperpm = hyperpm
        self.mode = hyperpm.marl_mode
        self.nmodal = hyperpm.nmodal
        ...
        if self.mode == 0:
            self.classifier = nn.Linear(d_hidden + self.nmodal ** 2, self.nclass)
        elif self.mode == 1:
            self.classifier = nn.Linear(self.nmodal ** 2, self.nclass)
        elif self.mode == 2:
            self.classifier = nn.Linear(d_hidden, self.nclass)

    def forward(self, x, attn_embedding):
        # x为attention的输出, attn_embedding即Vec(Pu)
        x = self.mlp_head(x)
        ...
        output_H = None
        if self.mode == 0:
            output_pred = self.classifier(combined_x)
            output_H = combined_x
        elif self.mode == 1:
            output_pred = self.classifier(attn_embedding)
            output_H = attn_embedding
        elif self.mode == 2:
            output_pred = self.classifier(x)
            output_H = x

        return F.log_softmax(output_pred, dim=1), output_H
